...
def _build_design_prompt(query: str, selected_anchors: list[dict]) -> str:
    ...
    lines.append("- **形状只考虑中心词对应的参考**（如‘皮’→rabbit_hide/leather 的皮张轮廓；‘稿’→pickaxe）；**修饰词参考（如 leatherworker 村民职业图标、emerald 宝石）只用于材质/颜色/细节，绝对不能当作形状**。")
    lines.append("- **先选定一个最贴合中心词的参考作为形状基础**；其他参考只用于材质/颜色/细节/装饰；然后在这个基础上**修改**（缩短/加长/换色/加装饰），不要拼合多个不同形状的轮廓。")
    lines.append("- **若中心词是‘皮/兽皮/皮革’**：形状以 rabbit_hide / leather 的**不规则皮张轮廓**（带毛边、边缘起伏）为主，不要用方形、柱状、门板/桌子/祭坛形状。")
...
def _build_selection_prompt(query: str, form: str, catalog_text: str, recommended: list[str]) -> str:
    ...
    sel_prompt = (
        ...
        "目标：%s（form=%s）\n"
        "下面是**简单关键词匹配的候选**（仅提示，可能不准/不完整；**请以你的语义理解为准**，可从全库任选）：\n%s\n\n"
        "再下面是**全库资源名**（你需要靠语义/常识判断哪些最贴合这个物品，尤其模组自定义、原版没有的东西更要自己推断）。\n"
        "请从中**选择 2-4 个**你最想参考的资产，并说明借什么。\n\n"
        "%s\n\n"
        "输出格式：\n"
        ...
        "> **先选定一个最贴合中心词的参考作为“形状基础”**；其余参考只用于材质/颜色/细节/装饰，不要拼合多个不同的形状轮廓。\n"

...
_PATTERN_INTENT: dict[str, tuple[list[str], str]] = {
    "眼球": (["eye 眼球（镶嵌于主体，3-4px 小区域）"],
             "眼球结构：白色/浅色眼白包围 + 绿色虹膜 + 深色瞳孔 + 1px 白色高光；眼球为 3-4px 圆/椭圆，嵌在弓弧中部，不是纯绿方块。"),
    "眼睛": (["eye 眼睛（3-4px 视觉焦点）"],
             "眼睛结构：浅色眼白 + 虹膜 + 深色瞳孔 + 1px 白高光；小区域镶嵌，不是纯色块。"),
    "眼": (["eye 眼睛（3-4px 视觉焦点）"],
          "眼睛结构：浅色眼白 + 虹膜 + 深色瞳孔 + 1px 白高光；小区域镶嵌，不是纯色块。"),
    "恶魔": (["flame 魂火/恶魔纹"],
...
GENERIC_PIXEL_DETAIL_RULES = [
    ...
    "纹样可缩放/可变形：局部装饰纹样（眼睛、宝石、火焰、花纹、徽记）无需按原尺寸复制，可缩放/变形；只借其**形态/层级**（如眼睛=眼白包围+虹膜+瞳孔+高光；宝石=棱面+高光），大小按目标部位调整，不逐像素照搬原图。",
